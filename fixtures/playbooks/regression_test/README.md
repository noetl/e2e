...
```bash
noetl run fixtures/playbooks/regression_test/create_test_schema.yaml
```
...
```bash
noetl run fixtures/playbooks/regression_test/regression_test.yaml
```
...
noetl register fixtures/playbooks/regression_test/master_regression_test.yaml \
  --host localhost --port 8082

# Execute test suite
noetl run catalog/fixtures/playbooks/regression_test/master_regression_test \
  -r distributed \
  --host localhost --port 8082 \
  --set pg_auth=pg_k8s
```
...
curl -X POST "http://localhost:8082/api/run/playbook" \
  ...
  -d '{
    "path": "fixtures/playbooks/regression_test/master_regression_test",
...
- step: test_my_playbook
  ...
  tool:
    kind: playbook
    path: fixtures/playbooks/my_category/my_playbook
  next:
...
- step: validate_my_playbook
  ...
  sink:
    ...
    data:
      test_run_id: "{{ workload.test_run_id }}"
      playbook_name: "{{ result.data.command_0.rows[0].test_name }}"
      playbook_path: fixtures/playbooks/my_category/my_playbook
...
1. **hello_world** (`fixtures/playbooks/hello_world`)
  ...
  test:
    ...
    steps:
      ...
      - name: Run regression tests
        run: noetl run fixtures/playbooks/regression_test/regression_test.yaml
...
1. **Expand Coverage**: Add remaining playbooks from `fixtures/playbooks/`
