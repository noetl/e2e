...
curl -X POST http://localhost:8082/api/catalog/register \
  ...
  -d '{
    "path": "fixtures/playbooks/json_serialization_save/json_serialization_save",
...
with open('fixtures/playbooks/json_serialization_save/json_serialization_save.yaml', 'r') as f:
    content = f.read()
r = requests.post('http://localhost:8082/api/catalog/register', 
                  json={'path': 'fixtures/playbooks/json_serialization_save/json_serialization_save',
                        'content': base64.b64encode(content.encode()).decode()})
print(r.status_code, r.text)
" && curl -X POST http://localhost:8082/api/execute \
  -H "Content-Type: application/json" \
  -d '{"path":"fixtures/playbooks/json_serialization_save/json_serialization_save"}'
```
