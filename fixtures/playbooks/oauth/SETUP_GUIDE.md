...
cat > fixtures/credentials/google_oauth.json << 'EOF'
{
  "name": "google_oauth",
  "type": "google_oauth",
...
.venv/bin/noetl execute playbook \
  "fixtures/playbooks/oauth/google_secret_manager" \
  --host localhost --port 8083

# Test GCS
.venv/bin/noetl execute playbook \
  "fixtures/playbooks/oauth/google_gcs" \
  --host localhost --port 8083
```
...
cat fixtures/credentials/google_oauth.json | jq .

# Test with gcloud
gcloud auth activate-service-account \
  --key-file=fixtures/credentials/google_oauth.json
