...
| `noetl run automation/playbooks/register.yaml --set path=fixtures/playbooks/container_postgres_init` | Register playbook with NoETL server |
| `noetl run fixtures/playbooks/container_postgres_init/container_postgres_init.yaml` | Execute the playbook |
...
```bash
# Build image
cd fixtures/playbooks/container_postgres_init
...
# Or manually
docker build -t noetl/postgres-container-test:latest fixtures/playbooks/container_postgres_init/
kind load docker-image noetl/postgres-container-test:latest --name noetl
```
...
curl -X POST "http://localhost:8082/api/credentials" \
  ...
  --data-binary @fixtures/credentials/pg_k8s.json
```
...
```bash
# Using noetl CLI (recommended)
noetl run automation/playbooks/register.yaml --set path=fixtures/playbooks/container_postgres_init

# Or directly via CLI
noetl register fixtures/playbooks/container_postgres_init/container_postgres_init.yaml \
  --host localhost --port 8082

# Or via script
./fixtures/register_test_playbooks.sh
```
...
```bash
# Using noetl CLI (recommended)
noetl run fixtures/playbooks/container_postgres_init/container_postgres_init.yaml

# Or using execute command
noetl execute playbook fixtures/playbooks/container_postgres_init \
  --host localhost --port 8082 \
  --json
