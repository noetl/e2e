...
if [ -n "$IMAGE_IN_CLUSTER" ]; then
    ...
    docker build -t noetl/postgres-container-test:latest fixtures/playbooks/container_postgres_init/ || log_fail "Build failed"
...
if curl -sf http://localhost:8082/api/credentials >/dev/null 2>&1; then
    ...
    if echo "$CREDS" | grep -q "pg_k8s"; then
        ...
        curl -X POST http://localhost:8082/api/credentials -H 'Content-Type: application/json' --data-binary @fixtures/credentials/pg_k8s.json || log_warn "Registration failed"
    fi
else
...
$CLI register fixtures/playbooks/container_postgres_init/container_postgres_init.yaml --host localhost --port 8082 || log_warn "Registration may have failed"
log_pass "Playbook registration attempted"

# === STEP 10: Execute Playbook ===
log_section "STEP 10: Execute Playbook"
echo "Executing playbook..."
EXEC_OUTPUT=$($CLI execute playbook fixtures/playbooks/container_postgres_init --host localhost --port 8082 --json 2>&1)
echo "$EXEC_OUTPUT"
