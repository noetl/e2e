...
- Parent playbook:
  `fixtures/playbooks/batch_execution/heavy_payload_pipeline_in_step_parallel/heavy_payload_pipeline_in_step_parallel.yaml`
...
noetl --host "$NOETL_HOST" --port "$NOETL_PORT" exec \
  catalog://fixtures/playbooks/batch_execution/heavy_payload_pipeline_in_step_parallel \
  -r distributed \
  --dry-run
```
...
RUN_JSON="$(curl -sS -X POST "http://$NOETL_HOST:$NOETL_PORT/api/execute" \
  ...
  -d '{
    "path": "fixtures/playbooks/batch_execution/heavy_payload_pipeline_in_step_parallel",
    "payload": {
