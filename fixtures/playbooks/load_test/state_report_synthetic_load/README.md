...
4. Provide a repeatable load-test playbook under the canonical `fixtures/playbooks/load_test/` area.

## Playbooks

- Main fixture: `fixtures/playbooks/load_test/state_report_synthetic_load/state_report_synthetic_load.yaml`
- Worker fixture: `fixtures/playbooks/load_test/state_report_synthetic_load/state_report_synthetic_load_worker.yaml`
...
curl -X POST http://localhost:8082/api/catalog/register \
  ...
  -d "$(jq -Rs '{content: .}' fixtures/playbooks/load_test/state_report_synthetic_load/state_report_synthetic_load_worker.yaml)"

curl -X POST http://localhost:8082/api/catalog/register \
  -H "Content-Type: application/json" \
  -d "$(jq -Rs '{content: .}' fixtures/playbooks/load_test/state_report_synthetic_load/state_report_synthetic_load.yaml)"
```
