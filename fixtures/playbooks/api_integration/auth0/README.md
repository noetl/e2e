...
- **`fixtures/gateway_ui/login.html`** - Frontend OAuth implementation
...
- `fixtures/gateway_ui/config.js` - Frontend configuration
...
with open("fixtures/playbooks/api_integration/auth0/provision_auth_schema.yaml") as f:
    content = f.read()
...
playbooks = glob.glob("fixtures/playbooks/api_integration/auth0/*.yaml")
...
cp fixtures/credentials/auth0_client.json.example \
   fixtures/credentials/auth0_client.json
...
- Credential file format matches other NoETL credentials (see `fixtures/credentials/`)
...
noetl credential register -f fixtures/credentials/pg_k8s.json
...
```bash
noetl credential register -f fixtures/credentials/pg_auth_user.json
```
