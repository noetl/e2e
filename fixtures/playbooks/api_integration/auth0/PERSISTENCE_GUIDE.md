...
- `fixtures/playbooks/api_integration/auth0/auth0_login.yaml` (v15)
...
- `fixtures/gateway_ui/login.html`
...
- `fixtures/gateway_ui/config.js` - Auth0 configuration
- `fixtures/gateway_ui/dashboard.html` - Dashboard page
- `fixtures/gateway_ui/auth.js` - Authentication utilities
- `fixtures/gateway_ui/app.js` - Main application logic
- `fixtures/gateway_ui/index.html` - Main page
- `fixtures/gateway_ui/styles.css` - Shared styles

### Kubernetes Manifests
- `ci/manifests/gateway/configmap-ui-files.yaml` - Generated ConfigMap with UI files
  - Generated with: `kubectl create configmap gateway-ui-files --from-file=fixtures/gateway_ui/ --namespace=gateway --dry-run=client -o yaml`
...
### Documentation
- `fixtures/playbooks/api_integration/auth0/README.md`
...
- UI source files (fixtures/gateway_ui/*)
...
curl -X POST http://localhost:8082/api/catalog/register \
  ...
  -d "{\"content\": \"$(cat fixtures/playbooks/api_integration/auth0/auth0_login.yaml)\"}"
...
If you modify UI files in `fixtures/gateway_ui/`, regenerate the ConfigMap:
...
curl -X POST http://localhost:8082/api/catalog/register \
  -H 'Content-Type: application/json' \
  -d "{\"content\": \"$(cat fixtures/playbooks/api_integration/auth0/provision_auth_schema.yaml)\"}"
...
curl -X POST http://localhost:8082/api/credentials \
  -H 'Content-Type: application/json' \
  -d @fixtures/credentials/auth0_client.json
