...
noetl register-credential --file fixtures/credentials/pg_k8s.json

# 2. Upload SQL script to GCS (if using remote script)
gsutil cp fixtures/playbooks/api_integration/auth0/sql/provision_auth_schema.sql \
  gs://noetl-scripts/auth0/

# 3. Register provisioning playbook
noetl register-playbook \
  --file fixtures/playbooks/api_integration/auth0/provision_auth_schema.yaml
...
cat > fixtures/credentials/pg_auth_user.json <<EOF
{
  "name": "pg_auth_user",
  "type": "postgres",
...
noetl register-credential --file fixtures/credentials/pg_auth_user.json
```
...
noetl register-playbook \
  --file fixtures/playbooks/api_integration/auth0/auth0_login.yaml

noetl register-playbook \
  --file fixtures/playbooks/api_integration/auth0/auth0_validate_session.yaml

noetl register-playbook \
  --file fixtures/playbooks/api_integration/auth0/check_playbook_access.yaml
```
