...
```bash
# Run the copy script
./fixtures/credentials/copy_gcloud_credentials.sh
...
3. **Copy the downloaded JSON**:
   ```bash
   cp ~/Downloads/your-project-id-xxxxx.json fixtures/credentials/google_oauth.json
   ```
...
5. **Register credential**:
   ...
   curl -X POST http://localhost:8083/api/credentials \
     ...
     --data-binary @fixtures/credentials/google_oauth.json
   ```
...
curl -X POST http://localhost:8083/api/credentials \
  -H 'Content-Type: application/json' \
  --data-binary @fixtures/credentials/nats_credential.json
```
...
5. **Test Authentication**:
   ...
   curl -X POST http://localhost:8083/api/credentials \
     -H 'Content-Type: application/json' \
     --data-binary @fixtures/credentials/ib_oauth.json
   
   # Execute OAuth test playbook
   .venv/bin/noetl execute playbook "fixtures/playbooks/oauth/interactive_brokers" \
...
**Reference**: 
- `fixtures/playbooks/oauth/interactive_brokers/OAUTH_IMPLEMENTATION.md`
- `fixtures/playbooks/oauth/interactive_brokers/README.md`
- IBKR OAuth API: `fixtures/playbooks/oauth/interactive_brokers/api-docs.json`
...
.venv/bin/noetl execute playbook "fixtures/playbooks/oauth/google_secret_manager" \
  --host localhost --port 8083
```
...
3. Saves to `fixtures/credentials/google_oauth.json`
