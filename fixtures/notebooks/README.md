...
3. Upload/open `fixtures/notebooks/regression_dashboard.py`.
4. Run cells top-to-bottom. Adjust at the top:
   - `PLAYBOOK_PATH` (default: `fixtures/playbooks/regression_test/master_regression_test.yaml`)
   - `HOURS_BACK`, `LIMIT`
...
The script reads Postgres connection parameters from `fixtures/credentials/pg_local.json` (in-cluster DNS):
